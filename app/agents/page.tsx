...
"Hi, can we move tomorrow's call to 4pm instead of 3pm? Sorry for the late notice."`}</Code>
      ...
      <Workshop
        title="‘회의록 정리 친구’ 5분 만에 만들기"
        duration="약 5분"
        difficulty="쉬움"
      >
        <Step n={1} title="클로드 코드에 만들어 달라고 부탁">
          <p>
            작업할 폴더에서 <code>claude</code>를 켠 뒤, 아래 부탁을 그대로
            붙여 넣어요.
          </p>
          <Code>{`이 폴더에 ".claude/agents/meeting-notes.md" 에이전트 파일을 만들어 줘.

성격: 회의 메모를 받아서 깔끔히 정리해 주는 친구.

답할 때 따라야 할 순서:
1) 한 줄 회의 요약
2) "결정 사항" — 불릿으로
3) "액션 아이템" — 담당자 / 마감일이 보이면 함께
4) "남은 질문" — 다음 회의에서 확인할 것

frontmatter의 description은 "회의 메모·녹취록을 받았을 때 호출되는 친구. 사용자가 '회의록 정리해 줘', '이 메모 깔끔하게', '액션 아이템 뽑아 줘' 같은 말을 하면 사용된다"라고 적어 줘.`}</Code>
          <p>
            클로드가 “파일을 만들어도 될까요?”라고 물으면 허락(Y) 하세요.
          </p>
        </Step>

        <Step n={2} title="진짜로 한 번 시켜 보기">
          <p>이번엔 실제 회의 메모를 던져요. 새 대화에서 이렇게:</p>
          <Code>{`아래 회의 메모 좀 정리해 줘.

— 메모 —
- 2시 시작, 마케팅·디자인·개발 참석
- 5월 캠페인 컨셉 두 개 후보 — A안(가족)/B안(혼자) 중 결정 필요
- 디자인 시안은 5/15까지 (지수)
- 문구 카피는 다음 주 미팅에서 다시 보기로
- 광고 예산은 아직 미정, 재무팀 확인 필요`}</Code>
        </Step>

        <Step n={3} title="잘 만들어졌다면">
          <p>
            클로드가 <strong>meeting-notes 친구</strong>를 호출해서 우리가
            정해 둔 순서(요약 → 결정 → 액션 → 질문)대로 답을 줍니다. 만약
            그냥 메인 클로드가 답한다면, 위쪽의{" "}
            <em>‘설명문(description)이 가장 중요해요’</em> 절을 다시 읽고,
            description의 말투에 ‘회의록 정리해 줘’ 같은 평소 말을 더 넣어
            보세요.
          </p>
        </Step>

        <Callout tone="tip" title="다음에 시도해 볼 만한 변형">
          <ul className="mt-1 list-disc pl-5 space-y-1">
            <li>‘담당자가 비어 있는 액션 아이템’을 따로 표시하게 하기</li>
            <li>30분·60분 회의 둘 중 무엇인지 첫 줄에 표시하기</li>
            <li>한국어/영어 메모 둘 다 받게 하기</li>
          </ul>
        </Callout>
      </Workshop>
